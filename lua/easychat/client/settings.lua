...
local function create_default_settings()
	...
	-- general settings
	do
		local category_name = "Основные"
		settings:AddCategory(category_name)

		settings:AddConvarSettingsSet(category_name, {
			[EC_ALWAYS_LOCAL] = "Всегда говорить в локальном режиме по умолчанию",
			...
			[EC_DISPLAY_TITLES] = "Отображать титулы игроков",
			[EC_DISPLAY_USERGROUPS] = "Отображать группы игроков",
			...
			[EC_TICK_SOUND] = "Звук тикания при новых сообщениях",
			[EC_USE_ME] = "Заменить ваше имя в чате на \"Я\"",
			[EC_GM_COMPLETE] = "Использовать автозаполнение стандартного игрового режима",
			[EC_NICK_COMPLETE] = "Автозаполнять имена игроков",
			[EC_NICK_PRIORITIZE] = "Приоритезировать автозаполнение имен игроков перед всем остальным",
			[EC_OUT_CLICK_CLOSE] = "Закрывать чат при клике в пустоту",
			[EC_SERVER_MSG] = "Отображать изменения серверных переменных",
			[EC_SKIP_STARTUP_MSG] = "Пропускать надоедливые сообщения при запуске аддонов",
			[EC_SYNC_STEAM_BLOCKS] = "Синхронизировать ваш список блокировок в Steam с EasyChat",
		})

		settings:AddSpacer(category_name)

		local setting_blocked_players = settings:AddSetting(category_name, "list", "Заблокированные пользователи")
		...
		local setting_blocked_strings = settings:AddSetting(category_name, "list", "Заблокированные слова")
		...
		blocked_strings_list:AddColumn("Контент")
		...
		settings:AddConvarSetting(category_name, "boolean", EC_TIMESTAMPS, "Отображать временные метки")
		settings:AddConvarSetting(category_name, "boolean", EC_TIMESTAMPS_12, "12-часовой формат временных меток")
		settings:AddConvarSetting(category_name, "boolean", EC_HUD_TIMESTAMPS, "Отображать временные метки в HUD-е чата")
		...
		local setting_disable_modules = settings:AddSetting(category_name, "action", EC_NO_MODULES:GetBool() and "Разрешить модули" or "Запретить модули")
		setting_disable_modules.DoClick = function() EC_NO_MODULES:SetBool(not EC_NO_MODULES:GetBool()) end

		local setting_reload_ec = settings:AddSetting(category_name, "action", "Перезагрузить EasyChat")
		setting_reload_ec.DoClick = function() EasyChat.Reload() end

		local setting_disable_ec = settings:AddSetting(category_name, "action", "Отключить EasyChat")
		...
		local function factory_reset()
			EasyChat.AskForValidation(
				...
				"Вы уверены, что хотите сбросить EasyChat до заводских настроек? Все ваши данные будут удалены.",
				{
					ok_text = "Сбросить",
		...
		local setting_factory_reset = settings:AddSetting(category_name, "action", "Сброс до заводских настроек")
	...
	-- chatbox settings
	do
		local category_name = "Окно чата"
		settings:AddCategory(category_name)

		settings:AddConvarSettingsSet(category_name, {
			[EC_GLOBAL_ON_OPEN] = "Открывать во вкладке глобального чата",
			[EC_HISTORY] = "Включить историю",
			[EC_PRESERVE_MESSAGE_IN_PROGRESS] = "Сохранять сообщения в процессе написания",
			[EC_TAGS_IN_CHATBOX] = "Отображать теги в окне чата",
			[EC_IMAGES] = "Отображать изображения",
			[EC_PEEK_COMPLETION] = "Предпросмотривать возможное автозаполнения чата",
			[EC_NON_QWERTY] = "Указать, есть ли у вас QWERTY клавиатура",
			[EC_BLUR_IMAGES] = "Размывать изображения в окне чата",
			[EC_BLUR_BACKGROUND] = "Размывать фон окна чата и его окон",
		})
		...
		if not EasyChat.UseDermaSkin then
			...
			local setting_outlay_color = settings:AddSetting(category_name, "color", "Цвет внешней панели")
			...
			local setting_outlay_outline_color = settings:AddSetting(category_name, "color", "Цвет контура внешней панели")
			...
			local setting_tab_color = settings:AddSetting(category_name, "color", "Цвет окон")
			...
			local setting_tab_outline_color = settings:AddSetting(category_name, "color", "Цвет контура окон")
		...
		tab_list.OnRowRightClick = function(_, _, line)
			...
			tab_menu:AddSpacer()
			tab_menu:AddOption("Запретить", function()
		...
		local setting_manage_tabs = settings:AddSetting(category_name, "action", "(АДМИН) Управление вкладками")
		...
		setting_manage_tabs.DoClick = function()
			...
			frame_tab_list:AddColumn("Название")
			...
			local setting_restrict_tab_name = settings:AddSetting(category_name, "string", "Название")
		...
		local setting_dermaskin = settings:AddSetting(category_name, "action", EC_USE_DERMASKIN:GetBool() and "Использовать современный скин" or "Использовать дерма-скин")
	...
	do
		...
		local setting_tags_names = settings:AddSetting(category_name, "boolean", "(АДМИН) Разрешить теги в именах")
		create_admin_shield_icon(setting_tags_names)

		local setting_tags_msgs = settings:AddSetting(category_name, "boolean", "(АДМИН) Разрешить теги в сообщениях")
		...
		settings:AddConvarSettingsSet(category_name, {
			[EC_HUD_FOLLOW] = "Следовать за окном чата",
			[EC_HUD_SMOOTH] = "Сглаживать переходы сообщений",
			[EC_HUD_SH_CLEAR] = "Очищать теги при вводе \"sh\"",
			[EC_HUD_CUSTOM] = "Использовать современный HUD чата",
		})

		settings:AddSpacer(category_name)

		settings:AddConvarSetting(category_name, "number", EC_HUD_WIDTH, "Ширина HUD-а", 1250, 250)
		settings:AddConvarSetting(category_name, "number", EC_HUD_POS_X, "Позиция HUD-а по X", 5000, 0)
		settings:AddConvarSetting(category_name, "number", EC_HUD_POS_Y, "Позиция HUD-а по Y", 5000, 0)

		local setting_reset_hud_bounds = settings:AddSetting(category_name, "action", "Сбросить границы HUD-а")
		...
		local setting_reset_duration = settings:AddSetting(category_name, "action", "Сбросить время жизни/затухания сообщения")
	...
	do
		...
		local setting_override_client_settings = settings:AddSetting(category_name, "boolean", "Серверные настройки переопределяют клиентские")
		...
		local function setup_rank(usergroup)
			...
			frame:SetTitle(usergroup and "Изменить ранг" or "Новый ранг")

			local setting_usergroup = settings:AddSetting(category_name, "string", "Группа")
			...
			local setting_tag = settings:AddSetting(category_name, "string", "Тег")
			...
			local setting_save = settings:AddSetting(category_name, "action", "Сохранить")
		...
		prefix_list.OnRowRightClick = function()
			...
			prefix_menu:AddOption("Изменить", modify_rank):SetImage("icon16/shield.png")
			prefix_menu:AddOption("Удалить", delete_rank):SetImage("icon16/shield.png")
		...
		local setting_add_usergroup = settings:AddSetting(category_name, "action", "(АДМИН) Добавить новый ранг")
		setting_add_usergroup:SetImage("icon16/shield.png")
		setting_add_usergroup.DoClick = function()
			setup_rank()
		end

		local setting_modify_usergroup = settings:AddSetting(category_name, "action", "(АДМИН) Изменить ранг")
		setting_modify_usergroup:SetImage("icon16/shield.png")
		setting_modify_usergroup.DoClick = modify_rank

		local setting_del_usergroup = settings:AddSetting(category_name, "action", "(АДМИН) Удалить ранг")
	...
	do
		...
		ollama_help.DoClick = function()
			...
			step1_desc2:SetText("Поместите загруженный .dll в папку garrysmod/lua/bin")
			...
			step1_arch_note:SetText("Установите x64 .dll только если вы используете ветку Garry's Mod x86-64.")
