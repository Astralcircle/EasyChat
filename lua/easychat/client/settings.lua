...
local function create_default_settings()
	...
	do
		...
		settings:AddConvarSettingsSet(category_name, {
			...
			[EC_NICK_PRIORITIZE] = "Давать приоритет автозаполнению никнеймов игроков перед всем остальным",
			[EC_OUT_CLICK_CLOSE] = "Закрывать чат при клике на пустоту",
			[EC_SERVER_MSG] = "Отображать изменения серверных переменных",
			[EC_SKIP_STARTUP_MSG] = "Пропускать надоедливые сообщения при запуске аддонов (показываются в консоли)",
