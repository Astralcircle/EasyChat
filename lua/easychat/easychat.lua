...
if CLIENT then
	...
	local function FormData()
		return {
			...
			Append = function(self, name, value, mime, filename)
				...
				table.insert(self.entries, {name = name, value = value, mime = mime, filename = filename})
			end,
			...
			GetHeaders = function(self)
				return {["Content-Length"] = #self:Read(), ["Content-Type"] = "multipart/form-data; charset=utf-8; boundary=" .. self.boundary}
			end,
		}
	end
	...
	function EasyChat.UploadToImgur(img_base64, callback)
		...
		form:Append("fileToUpload", util.Base64Decode(img_base64), "image.png")
